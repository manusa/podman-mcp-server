...
npm-copy-project-files: npm-copy-binaries ## Copy the project files to the main npm package and generate all package.json files
	...
	@echo '"description": "Model Context Protocol (MCP) server for container runtimes (Podman and Docker)",' >> $(MAIN_PACKAGE_JSON)
	...
	@echo '"repository": {"type": "git", "url": "git+https://github.com/manusa/podman-mcp-server.git"},' >> $(MAIN_PACKAGE_JSON)
	@echo '"keywords": ["mcp","podman","docker","container","model context protocol","model","context","protocol"],' >> $(MAIN_PACKAGE_JSON)
	@echo '"author": {"name": "Marc Nuri", "url": "https://www.marcnuri.com"},' >> $(MAIN_PACKAGE_JSON)
	@echo '"license": "Apache-2.0",' >> $(MAIN_PACKAGE_JSON)
	@echo '"bugs": {"url": "https://github.com/manusa/podman-mcp-server/issues"},' >> $(MAIN_PACKAGE_JSON)
	@echo '"homepage": "https://github.com/manusa/podman-mcp-server#readme",' >> $(MAIN_PACKAGE_JSON)
	@echo '"mcpName": "io.github.manusa/podman-mcp-server"' >> $(MAIN_PACKAGE_JSON)
	@echo '}' >> $(MAIN_PACKAGE_JSON)
	$(foreach os,$(OSES),$(foreach arch,$(ARCHS), \
		...
		echo '"description": "Model Context Protocol (MCP) server for container runtimes (Podman and Docker)",' >> $$OS_PACKAGE_JSON; \
		echo '"repository": {"type": "git", "url": "git+https://github.com/manusa/podman-mcp-server.git"},' >> $$OS_PACKAGE_JSON; \
		OS="$(os)"; \
